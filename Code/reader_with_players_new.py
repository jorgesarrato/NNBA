...
class Reader_Polars:
	def __init__(self, path, player_path, latest_matches=5, latest_vs_matches=2, latest_other_matches=5, latest_player_dates = 5, predict_path=None):
		...
		self.other_game_fields_orig = [
			"pts_home", "pts_away", "fg_home","fg_att_home","3pt_home","3pt_att_home","ft_home","ft_att_home","oreb_home","dreb_home","ast_home","stl_home","blk_home","to_home",
			"wins_at_home_HOME", "wins_away_HOME", "losses_at_home_HOME", "losses_away_HOME",
			"pf_home","fg_away","fg_att_away","3pt_away","3pt_att_away","ft_away","ft_att_away","oreb_away","dreb_away","ast_away","stl_away","blk_away","to_away","pf_away", "MIN",
			"wins_at_home_AWAY", "wins_away_AWAY", "losses_at_home_AWAY", "losses_away_AWAY"

		]
		self.other_game_fields_inverted = [
			"pts_away", "pts_home", "fg_away","fg_att_away","3pt_away","3pt_att_away","ft_away","ft_att_away","oreb_away","dreb_away","ast_away","stl_away","blk_away","to_away",
			"wins_at_home_AWAY", "wins_away_AWAY", "losses_at_home_AWAY", "losses_away_AWAY",
			"pf_away","fg_home","fg_att_home","3pt_home","3pt_att_home","ft_home","ft_att_home","oreb_home","dreb_home","ast_home","stl_home","blk_home","to_home","pf_home", "MIN",
			"wins_at_home_HOME", "wins_away_HOME", "losses_at_home_HOME", "losses_away_HOME"
		]

		self.player_fields = ["mp", "fg", "fg_att", "3pt", "3pt_att", "ft", "ft_att", "oreb", "dreb", "ast", "stl", "blk", "to", "pf", "pts", "+/-", "START_POSITION", 'days_since_date']
	...
	def make_dataset(self, only_new_games=False):
		...
		self.data = self.data.join(
			self.data2.select(["Year", "Month", "Day", "home", "away", "home_close_ml", "away_close_ml", "close_over_under", "home_close_spread"]),
		...
		self.data = self.data.with_columns(
			...
			.then(col("OverUnder"))
			.when((col("close_over_under") != 0) & (col("close_over_under").is_not_null()))
			.then(col("close_over_under"))
			.otherwise(lit(float('nan')))
			.alias("OverUnder")
		)
